...
const $$Navigation = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<li${addAttribute(hasSubnav ? "dropdown" : "", "class")}${addAttribute(navigation.id, "key")}> <a class="nav-link scrollto"${addAttribute(`${navigation.slug}`, "href")}><span>${navigation.name}</span> ${subnav && subnav.length > 0 && renderTemplate`<i class="bi bi-chevron-down"></i>`} </a> ${subnav && subnav.length > 0 && renderTemplate`<ul> ${subnav.map((subnavItem) => renderTemplate`<li><a${addAttribute(`${subnavItem.slug}`, "href")}>${subnavItem.name}</a></li>`)} </ul>`} </li>`;
...
const $$AssistantInstructors = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Club Instructors" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="inner-page"> <div class="container"> <br><br><br><br> <h1>Assistant Instructors</h1> <p>
...
export { $$Layout as $, assistantInstructors as a, masters as m };
